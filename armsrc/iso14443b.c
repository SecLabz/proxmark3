...
static int st25tb_tear_off_read_block(uint8_t block_address, uint32_t *block_value, int pre_read_delay_us) {
    int res;
    iso14b_card_select_t card;
    iso14443b_setup();

    SpinDelayUsPrecision(pre_read_delay_us*1000);
    ...
    if(res == PM3_SUCCESS) {
        *block_value = (block[3] << 24) | (block[2] << 16) | (block[1] << 8) | block[0];
    }
...
const int TEAR_OFF_START_OFFSET_US = 150;
const int TEAR_OFF_ADJUSTMENT_US = 25;
const int PRE_READ_DELAY_US = 0;


static int write_14b_st_block(uint8_t blocknr, uint8_t *block) {

    uint8_t cmd[] = {ISO14443B_WRITE_BLK, blocknr, block[0], block[1], block[2], block[3], 0x00, 0x00};
    AddCrc14B(cmd, 6);
...
void st25tb_tear_off_write_block(uint8_t blocknr, uint32_t data, uint16_t tearoff_delay_us) {
    iso14443b_setup();
    ...
    res = write_14b_st_block(blocknr, block);
    tearoff_hook();
...
int8_t st25tb_tear_off_retry_write_verify(uint8_t block_address, uint32_t target_value, uint32_t max_try_count, int sleep_time_ms, uint32_t *read_back_value)
{
    int i = 0;
    ...
    while (*read_back_value != (target_value) && i < max_try_count)
    {
        ...
        st25tb_tear_off_read_block(block_address, read_back_value, 0);
        SpinDelayUsPrecision(sleep_time_ms*1000);
        i++;
    }
...
void st25tb_tear_off_log(int tear_off_us, char *color, uint32_t value)
{
    ...
    for (int i = 31; i >= 0; i--) {
        binaryRepresentation[31 - i] = (value & (1 << i)) ? '1' : '0';
    }
    binaryRepresentation[32] = '\0';
    Dbprintf("%s%08X%s : %s%s%s : %02d us\n", color, value, RESET, color, binaryRepresentation, RESET, tear_off_us);
}
...
void write_14b_st_counter(iso14b_st_write_counter_t *p) {
    ...
    uint8_t block_address = p->blockno;
    uint32_t target_value = (p->data[0] << 24) | (p->data[1] << 16) | (p->data[2] << 8) | p->data[3];

    uint32_t read_value = 0;
    uint32_t current_value = 0;
    uint32_t last_consolidated_value = 0;
    uint32_t tear_off_value = 0;
    uint32_t tear_off_adjustment_us = p->adjustment_us;
    uint32_t safety_value = (p->safety_value[0] << 24) | (p->safety_value[1] << 16) | (p->safety_value[2] << 8) | p->safety_value[3];
    ...
    result = st25tb_tear_off_read_block(block_address, &current_value, 0);
    if (result != 0)
    {
        Dbprintf("Tag not found");
        reply_ng(CMD_HF_ST_WRITE_COUNTER, PM3_EOPABORTED, NULL, 0);
        return;
    }
    ...
    Dbprintf("Adjustment us: %d", tear_off_adjustment_us);


    if (tear_off_value == 0)
    {
        Dbprintf("Tear off technique not possible.");
        reply_ng(CMD_HF_ST_WRITE_COUNTER, PM3_EOPABORTED, NULL, 0);
        return;
    }
    ...
    for (;;)
    {
        ...
        result = st25tb_tear_off_read_block(block_address, &read_value, PRE_READ_DELAY_US);
        if (result != 0)
        {
            continue;
        }
        ...
        if (read_value > current_value)
        {
                ...
                ((read_value & 0xF0000000) > (current_value & 0xF0000000)))
            {
                ...
                {
                    Dbprintf("Success !");
                    reply_ng(CMD_HF_ST_WRITE_COUNTER, result, NULL, 0);
                    return;
                }
